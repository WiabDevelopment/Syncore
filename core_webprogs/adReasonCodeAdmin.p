...
DEFINE VARIABLE chrSelectedReasonTable  AS CHARACTER NO-UNDO INITIAL "1".
...
&IF DEFINED(EXCLUDE-pBuildBrowse) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pBuildBrowse Procedure 
PROCEDURE pBuildBrowse :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
   ...
   RUN pDeleteReasonTempTableContent.

   /* This finds the ProcessEvent which is linked to the form that is passed in */
   {webGetFormProcessEvent.i "reason_details_form"} 
   ...
   DO:
      ...
      DO:
         ...
         REPEAT WHILE NOT hdlTempReasonQuery:QUERY-OFF-END:
            ...
            CREATE QUERY hdlTempRuleQuery NO-ERROR.
            
            hdlTempRuleQuery:ADD-BUFFER(hdlTempRuleBuffer).
            logRuleQueryPrepareOK = hdlTempRuleQuery:QUERY-PREPARE("FOR EACH " + chrSelectedRuleTable + " WHERE " 
                                                                   + chrSelectedRuleTable + "." + chrSelectedRuleTable + "ID = " 
                                                                   + STRING(hdlTempReasonBuffer:BUFFER-FIELD(chrSelectedRuleTable + "ID"):BUFFER-VALUE)) NO-ERROR.
            IF logRuleQueryPrepareOK THEN
            DO:
               hdlTempRuleQuery:QUERY-OPEN().
               hdlTempRuleQuery:GET-FIRST(NO-LOCK).
               
               ASSIGN ttReason.RuleID       = INTEGER(STRING(hdlTempRuleBuffer:BUFFER-FIELD(chrSelectedRuleTable + "ID"):BUFFER-VALUE)).
                      ttReason.FromStatusID = INTEGER(hdlTempRuleBuffer:BUFFER-FIELD("FromStockStatusID"):BUFFER-VALUE).
...
&IF DEFINED(EXCLUDE-pBuildBrowseButtonBar) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pBuildBrowseButtonBar Procedure 
PROCEDURE pBuildBrowseButtonBar :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
   ...
   ReasonBrowseButtons:addButton("reason_browse_form_btn_create",
                                 ...
                                 (IF chrSelectedReasonTable = "1" THEN "Disabled" ELSE "")).
...
&IF DEFINED(EXCLUDE-pBuildSelectionForm) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pBuildSelectionForm Procedure 
PROCEDURE pBuildSelectionForm :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
   ...
   ReasonSelectionForm:FormAction  = "adReasonCodeAdmin.p?" + TRIM(chrGblDefaultUrlValues,"&").
   ...
   ReasonSelectionForm:insertComboPairs("ReasonTableName", "1", "Choose a table...").
...
&IF DEFINED(EXCLUDE-pDeleteReasonTempTableContent) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pDeleteReasonTempTableContent Procedure 
PROCEDURE pDeleteReasonTempTableContent :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
&IF DEFINED(EXCLUDE-pDisplayObjectsOnMainFrame) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pDisplayObjectsOnMainFrame Procedure 
PROCEDURE pDisplayObjectsOnMainFrame :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
   ...
   MainFrame = NEW pageFrame().
   MainFrame:WebStream = STREAM WebStream:HANDLE.
   
   /* creating a new frame */
   MainFrame:frameOpen(985, 70, "").
   ...
   /* Insert a form */
   ReasonBrowseFrame:FormAction="dbReasonCodeUpdate.p?" + TRIM(chrGblDefaultUrlValues,"&").
   ...
   ReasonBrowseFrame:insertHiddenField("prog_name", "adReasonCodeAdmin.p").
...
------------------------------------------------------------------------------*/
   ...
   ASSIGN chrDisplayFieldList  = chrSelectedReasonTable + "ID,ListingSequence,ReasonName,ReasonDetails,Active"
          chrEditFieldList     = "ListingSequence,ReasonName,ReasonDetails,Active"
          chrNewFieldList      = "ListingSequence,ReasonName,ReasonDetails,Active"
   ...
   ReasonDetailsForm:FormAction = "dbReasonCodeUpdate.p?" + TRIM(chrGblDefaultUrlValues,"&").
   ...
   ReasonDetailsForm:insertTextField("ReasonDetails", "", 200, TRUE).  
   ...
   ReasonDetailsForm:insertHiddenField("prog_name", "adReasonCodeAdmin.p").
...
------------------------------------------------------------------------------*/
   ...
   IF chrSelectedReasonTable = "" THEN chrSelectedReasonTable = "1".
   ...
   ThisPage:addJavaScript("/syngate/" + LOWER(Config.PackageName) + "/js/ReasonCode.js"). /* Append your file here */
   ...
   RUN pBuildSelectionForm.
   
   RUN pBuildBrowse.
   
   RUN pBuildBrowseButtonBar.
   
   RUN pDisplayObjectsOnMainFrame.
