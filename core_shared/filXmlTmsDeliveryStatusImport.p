/*------------------------------------------------------------------------------------------------------------------------------------------
Program : filXmlTmsDeliveryStatusImport.p
Purpose :  
...
DEFINE VARIABLE intInternalEmailGroup          AS INTEGER   NO-UNDO.
...
DO:
   ASSIGN chrErrorDetails = "GateUser with Username: WebServ does not exist!"
          logErrorFound = TRUE NO-ERROR.
END.  
...
DO ON ERROR UNDO, RETURN ERROR:
   ...
   DO:
      ASSIGN chrErrorDetails = "FileMaster with GblFileMasterID: " + STRING(intGblFileMasterID) + " is not available!"
             logErrorFound = TRUE NO-ERROR.
      LEAVE Main_Block.
   END.
   ...
   IF intFileCompletedStatus = 0 THEN
      RETURN ERROR "No FileStatus exists for Code: Complete".
      
   FIND FIRST EmailGroup NO-LOCK
      WHERE FileMaster.InternalEmailGroupID = EmailGroup.EmailGroupID NO-ERROR.
      
   IF AVAILABLE EmailGroup THEN
      intInternalEmailGroup = EmailGroup.EmailGroupID.
   ELSE
      intInternalEmailGroup = 1.
   ...
   DO:
      ASSIGN chrErrorDetails = "FileType with GblFileMasterID: " + STRING(intGblFileMasterID) + " is not available!"
             logErrorFound = TRUE NO-ERROR.
      LEAVE Main_Block.
   END.
   ...
   DO:
      ASSIGN chrErrorList = chrErrorList + " No ShipOrderStatus for StatusCode: DeliveryFailed."
             logErrorFound = TRUE NO-ERROR.
   END.
   ...
   DO:
      ASSIGN chrErrorList = chrErrorList + " No ShipOrderStatus for StatusCode: Delivery."
             logErrorFound = TRUE NO-ERROR.
   END.
   ...
   DO:
      ASSIGN chrErrorList = chrErrorList + " No records in Shipment Status file."
             logErrorFound = TRUE NO-ERROR.
   END.
   
   FIND FIRST ttStatusDetail NO-LOCK NO-ERROR.
   IF AVAILABLE ttStatusDetail THEN
   DO:
   ...
   DO:
      ASSIGN chrErrorList = chrErrorList + "Reference Number Data Type: SyncreonOrderNo is NOT in the Delivered Milestone XML from TMS."
             logErrorFound = TRUE NO-ERROR.
   END.
   ELSE
   DO:
      ...
      DO:
         ASSIGN chrErrorList = chrErrorList + "Ship Order Ref: " + STRING(ttReferenceNumber.ReferenceNumber) + " in the Delivered Milestone from TMS does NOT Exist."
                logErrorFound = TRUE NO-ERROR.
      END.
      ELSE
      DO:
         /* Check Order has been ShipConfirmed */
         IF ShipOrder.ShipConfirmed = "" THEN
         DO:
            ASSIGN chrErrorList = chrErrorList + "Ship Order Ref: " + ShipOrder.OrderRef + " has NOT been ShipConfirmed."
                   logErrorFound = TRUE NO-ERROR.      
         END.
      END.   
   END.   
   ...
   DO:
      ASSIGN chrDeliveryDay     = SUBSTRING(ttStatusDate.DateInfo, 4, 2)
             ...
             chrDeliveryMinutes = SUBSTRING(ttStatusDate.DateInfo, 15, 2) NO-ERROR.
   END.   
         ...
         DO:
            ...
            DO:
               RUN pSendErrorEmail(INPUT fGetLockingInfo(INTEGER(RECID(ShipOrder)),
                                   ...
                                   INPUT "ShipOrder Record " + ShipOrder.OrderRef + " is locked. Undoing update.",
                                   INPUT chrGblFilePath,
                                   INPUT intInternalEmailGroup).
