/*------------------------------------------------------------------------------------------------------------------------------------------
Program : filXmlTmsPickedStatusExport.p
Purpose : 
...
DEFINE BUFFER pickPackShipOrderStatus FOR ShipOrderStatus.
...
FIND FIRST FileMaster NO-LOCK /*idx=FileMasterID*/
   WHERE FileMaster.MasterName = "TmsPickedStatusUpdate" NO-ERROR.
IF NOT AVAILABLE FileMaster THEN 
DO:
   chrReturnError = "FileMaster: TmsPickedStatusUpdate does not exist!".
...
chrLogFile = chrLogFileDirectory + chrFileMasterName + "_filXmlTmsPickedStatusExport_" + fDisplayDate&Time(fTimestamp(NOW),"d_m_y") + ".log".
...
FIND FIRST OperationType NO-LOCK /*idx=TypeCode*/
   WHERE OperationType.TypeCode = "TmsPickStatusUpdate".
IF NOT AVAILABLE OperationType THEN
DO:
   RETURN ERROR "NO OperationTypeCode for TmsPickStatusUpdate".
END. /*IF NOT AVAILABLE OperationType THEN*/

intGblOperationTypeID = OperationType.OperationTypeID. 

FIND FIRST EmailGroup NO-LOCK
   WHERE EmailGroup.GroupCode = "TmsPickedStatusUpdate" NO-ERROR.
...
DO:
   ...
   IF logGblDebugging THEN 
      fLog("Running: filXmlTmsPickedStatusUpdate.p").
   ...
   FIND FIRST pickPackShipOrderStatus NO-LOCK /*idx=StatusCode*/
      WHERE pickPackShipOrderStatus.StatusCode = "PackedOut" NO-ERROR.
   IF NOT AVAILABLE pickPackShipOrderStatus THEN
   DO:
      chrReturnError = "ShipOrderStatus: PackedOut, does not exist!".
      RETURN ERROR chrReturnError. 
   ...
   FOR EACH ShipOrder NO-LOCK /*idx=ShipOrderTypeId*/
      ...
      AND   ShipOrder.CarrierPickConfirmed = ""
      AND   ShipOrder.PickConfirmed       <> "":
          
      /* Originally the For Each had this ---
         AND   ShipOrder.ActualsMessageSent  <> ""
      */
      ...
      DO TRANS ON ERROR UNDO, LEAVE:
         ...
         ASSIGN ttStatusDetail.statusReasonCode = "ORDER PACKED" 
                ttStatusDetail.statusCode       = "PK".
         ...
         DO ON ERROR UNDO OrderBlk, LEAVE OrderBlk:
            ...
            ASSIGN updShipOrder.CarrierPickConfirmed = fTimestamp(NOW).
