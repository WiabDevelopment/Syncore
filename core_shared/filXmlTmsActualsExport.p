/*------------------------------------------------------------------------------------------------------------------------------------------
Program : filXmlTmsActualsExport.p
Purpose : Exports Actual data calculated by the BoxLogic into TMS XML files
...
DEFINE TEMP-TABLE ttLoad XML-NODE-NAME "Load"
   FIELD action        AS CHARACTER XML-NODE-TYPE "Attribute" INITIAL "Update" XML-NODE-NAME "action"
   FIELD MercuryGateID AS INTEGER   XML-NODE-TYPE "Hidden"
...
DEFINE TEMP-TABLE ttLoadReferenceNumber XML-NODE-NAME "ReferenceNumber"
   FIELD LoadReferenceNumbersID AS INTEGER   XML-NODE-TYPE "Hidden"
   FIELD RefNumberID            AS INTEGER   XML-NODE-TYPE "Hidden"
   ...
   INDEX ReferenceNumber IS UNIQUE LoadReferenceNumbersID RefNumberID.
...
DEFINE TEMP-TABLE ttLoadShipment XML-NODE-NAME "Shipment"
   ...
   FIELD ShipmentAction AS CHARACTER XML-NODE-TYPE "Attribute" INITIAL "Update" XML-NODE-NAME "action"
   FIELD LoadID         AS INTEGER   XML-NODE-TYPE "Hidden"
   FIELD ShipmentID     AS INTEGER   XML-NODE-TYPE "Hidden"
...
DEFINE TEMP-TABLE ttItemReferenceNumber XML-NODE-NAME "ReferenceNumber"
   FIELD RefType         AS CHARACTER XML-NODE-TYPE "Attribute" INITIAL "TrackingNumber" XML-NODE-NAME "type"
   FIELD IsPrimary       AS CHARACTER XML-NODE-TYPE "Attribute" INITIAL "false" XML-NODE-NAME "isPrimary"
   FIELD ItemGroupID     AS INTEGER   XML-NODE-TYPE "Hidden"
   FIELD ReferenceNumber AS CHARACTER XML-NODE-TYPE "Text"
   INDEX ItemGroupID ItemGroupID.
...
DEFINE VARIABLE intErrorEmailGroupID           AS INTEGER   NO-UNDO.
...
DO:
   ...
   FIND FIRST FileMaster NO-LOCK 
      WHERE FileMaster.MasterName = "tmsShipPackageActuals" NO-ERROR.
   IF NOT AVAILABLE FileMaster THEN 
   DO:
      chrReturnError = "FileMaster: tmsShipPackageActuals does not exist!".
   ...
   IF logGblDebugging THEN 
   DO:
      fLog("Running: filXmlTmsActualsExport.p").
   END.
   ...
   CREATE ttLoadReferenceNumbers.  
          ttLoadReferenceNumbers.LoadReferenceNumbersID = 1.
   
   CREATE ttLoadReferenceNumber. 
   ASSIGN ttLoadReferenceNumber.LoadReferenceNumbersID = 1
          ttLoadReferenceNumber.RefNumberID            = 1
          ttLoadReferenceNumber.RefType                = "Load ID"
          ttLoadReferenceNumber.IsPrimary              = "true"
          ttLoadReferenceNumber.ReferenceNumber        = "" NO-ERROR.   
   
   CREATE ttLoadReferenceNumber. 
   ASSIGN ttLoadReferenceNumber.LoadReferenceNumbersID = 1
          ttLoadReferenceNumber.RefNumberID            = 2
          ttLoadReferenceNumber.RefType                = "SyncreonOrderNo"
          ttLoadReferenceNumber.IsPrimary              = "false"
          ttLoadReferenceNumber.ReferenceNumber        = "" NO-ERROR.
   ...
   FOR EACH ShipOrder NO-LOCK 
      WHERE ShipOrder.PickConfirmed <> "" AND ShipOrder.ActualsMessageSent = "": 
      
      /* Clean dynamically populated data from tt */
      RUN pEmptyDynamicTTData.
      
      /* Reset PackageSequence */
      ASSIGN intPackageSequenceCounter = 1           /* Resets Package Counter */
             decTotalPackageWeight     = 0           /* Resets weight accumulator */            
             intNumberOfSkus           = 0 NO-ERROR. /* Resets LiteItem Counter */
      ...
      DO TRANS ON ERROR UNDO, LEAVE:
         ...
         /* Dynamic data for PickUp Event Address */
         ...
         DO:
            RUN pCreateFileError(INPUT "No Address found for OrderRef: " + 
         ...
         /* Dynamic data for Drop Event Address */
         CREATE ttEventAddress. 
         ...
         FOR EACH ShipPackage OF ShipOrder NO-LOCK:
            ...
            FIND FIRST ShipPackageType OF ShipPackage NO-LOCK NO-ERROR.
            IF NOT AVAILABLE ShipPackageType THEN
            DO:
               RUN pCreateFileError(INPUT "No ShipPackageType found with ShipPackageTypeID: " + 
                                          STRING(ShipPackage.ShipPackageTypeID)               +
                                          " for OrderRef: "                                   +
                                          STRING(ShipOrder.OrderRef)                          +
                                          " PackageRef: " + ShipPackage.PackageRef).               
               UNDO ShipOrderBlk, LEAVE ShipOrderBlk. 
            END.
             
            FIND FIRST ShipPackageSize NO-LOCK OF ShipPackageType NO-ERROR.
            IF NOT AVAILABLE ShipPackageSize THEN
            DO:
               RUN pCreateFileError(INPUT "No ShipPackageSize found with ShipPackageSizeID: " + 
                                          STRING(ShipPackageType.ShipPackageTypeID)           +
                                          " for OrderRef: "                                   +
                                          STRING(ShipOrder.OrderRef)                          +
                                          " PackageRef: " + ShipPackage.PackageRef).              
               UNDO ShipOrderBlk, LEAVE ShipOrderBlk. 
            ...
            FOR EACH ShipPackageLine OF ShipPackage NO-LOCK:
               ...
               FIND FIRST Part OF ShipPackageLine NO-LOCK NO-ERROR.
               IF NOT AVAILABLE Part THEN
               DO:
                  RUN pCreateFileError(INPUT "No Part found with PartID: "  + 
                                             STRING(ShipPackageLine.PartID) +
                                             " for OrderRef: "              +
                                             ShipOrder.OrderRef).                   
                  UNDO ShipOrderBlk, LEAVE ShipOrderBlk.    
               END.
               ...
               DO:
                  RUN pCreateFileError(INPUT "Weight cannot be 0. PartRef " + Part.PartRef +
                                             " for OrderRef: "              + ShipOrder.OrderRef).
                  UNDO ShipOrderBlk, LEAVE ShipOrderBlk.     
               END.
                           
               CREATE ttItemGroup. 
               ASSIGN ttItemGroup.ItemGroupID       = intItemGroupIDCounter
                      ttItemGroup.GroupID           = STRING(ShipPackageLine.ShipPackageLineID)
               ...
               ASSIGN ttContainedBy.ItemGroupID   = intItemGroupIDCounter
                      ttContainedBy.ContainedByID = ShipPackage.PackageRef NO-ERROR.
               ...
               decTotalPackageWeight = decTotalPackageWeight + 
                                       ( Part.UnitWeight * ShipPackageLine.LineQty).
               ...
               ASSIGN ttQuantity.ItemGroupiD  = intItemGroupIDCounter
                      ...
                      ttQuantity.Quantity     = STRING(ShipPackageLine.LineQty) NO-ERROR.
               ...
               ASSIGN intItemGroupIDCounter = intItemGroupIDCounter + 1
                      intNumberOfSkus       = intNumberOfSkus + 1 NO-ERROR. /* counter of LitemItems */
            ...
            ASSIGN ttItemGroup.ItemGroupID       = intItemGroupIDCounter
                   ...
                   ttItemGroup.GroupID           = ShipPackage.PackageRef
                   ...
                   ttItemGroup.StccCode          = chrItemGroupSTCCCode
                   ttItemGroup.HazardousMaterial = "false" 
                   ttItemGroup.GroupDescription  = ShipPackage.PackageRef NO-ERROR.
            ...
            ASSIGN ttItemReferenceNumber.ItemGroupID     = intItemGroupIDCounter
                   ttItemReferenceNumber.ReferenceNumber = ShipPackage.CarrierTrackingRef NO-ERROR. /* Assigning TrackingNumber */
            ...
            FIND FIRST ttLoadReferenceNumber 
               WHERE ttLoadReferenceNumber.RefType = "SyncreonOrderNo" NO-ERROR.
            ttLoadReferenceNumber.ReferenceNumber = ShipOrder.OrderRef.
            
            FIND FIRST ttLoadReferenceNumber 
               WHERE ttLoadReferenceNumber.RefType = "Load ID" NO-ERROR.
            ttLoadReferenceNumber.ReferenceNumber = ShipOrder.LoadRef.
            ...
            DO:
               RUN pCreateFileError(INPUT "No PickUp Event found for OrderRef: " +
                                          STRING(ShipOrder.OrderRef)).              
               UNDO ShipOrderBlk, LEAVE ShipOrderBlk. 
            END.
            ...
            IF NOT AVAILABLE ttEventReferenceNumber THEN
            DO:
               RUN pCreateFileError(INPUT "No Drop Event found for OrderRef: " +
                                          STRING(ShipOrder.OrderRef)).              
               UNDO ShipOrderBlk, LEAVE ShipOrderBlk. 
            END.
            ...
            /* increment counter again, if there are more than one package units */
         ...
         END. /* End of for each ShipPackage */
         
         IF intNumberOfSkus = 0 THEN
         DO:
            RUN pCreateFileError(INPUT "No parts were found in the ShipPackageLine for OrderRef: " +
                                       STRING(ShipOrder.OrderRef)).            
            UNDO ShipOrderBlk, LEAVE ShipOrderBlk. 
         END.  
         
         /* update ShipOrder ActualsMessageSent */
         ...
         updShipOrder.ActualsMessageSent = fTimeStamp(NOW).
         
         /* check if updShipOrder update was successful*/
         ...
         /* Save MSG for each ShipOrder that has at least one LineItem */
      ...
      DO:
         ...
         RUN osSendMail.p (INPUT chrEmailAddress,                              /* Optional list of Users */
                           INPUT ttFileExport.FileName + " File Upload Error", /* Email Subject */
   ...
   END. /* End of for each ShipOrderLoop */   
END. /* End of MainBlk */     
