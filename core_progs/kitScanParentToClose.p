/*------------------------------------------------------------------------------------------------------------------------------------------
Program : kitScanParentToClose.p
Purpose : Scan a Kitting Stock Package Parent that needs to be closed.
                    
          Possible outcomes : Continue.
          
Author  : BR
Date    : 15th Oct 2015
--------------------------------------------------------------------------------------------------------------------------------------------
Changes :
--------------------------------------------------------------------------------------------------------------------------------------------
------------------------------------------------------------------------------------------------------------------------------------------*/

/* Required to prevent CTRL-C or CRTL-BREAK */
DO ON STOP UNDO, RETRY:
   
   /* Character Parameter Include */
   {defProcessParameters.i}
   
   /* Standard Mandatory Includes */
   {defSessionVariables.i}
   {fncClassFunctions.i}
   {fncGlobalFunctions.i}   
   {fncStringFunctions.i}   
   
   /* Scan Gun Message/Confirm */
   {prcScannerMessaging.i}
   
   /* Status Functions */
   {fncStatusTypeFunctions.i}
   
   /* Kitting Dependencies */  
   {fncDateFunctions.i}      
   {fncLoggingFunctions.i}     
   {prcKittingProcedures.i}
   {fncKittingFunctions.i}
   
   /* Map Debugging */
   {prcProcessDebugging.i}
  
   /* Local Variables */
   DEFINE VARIABLE chrScannedPackage      AS CHARACTER NO-UNDO FORMAT "x(24)" VIEW-AS FILL-IN NATIVE.
   DEFINE VARIABLE chrUserPrompt          AS CHARACTER NO-UNDO FORMAT "x(24)" VIEW-AS FILL-IN NATIVE.
   DEFINE VARIABLE intReturnedWorkOrderID AS INTEGER NO-UNDO.
   
   /* Session Objects */
   DEFINE VARIABLE chrSsnLastScan        AS sessionValue NO-UNDO.
   DEFINE VARIABLE chrSsnUserPrompt      AS sessionValue NO-UNDO.
   DEFINE VARIABLE chrSsnLabelType       AS sessionValue NO-UNDO.
   DEFINE VARIABLE logSsnTempLabel       AS sessionValue NO-UNDO.
   DEFINE VARIABLE chrSsnLabelName       AS sessionValue NO-UNDO.
   DEFINE VARIABLE intSsnParentPackageID AS sessionValue NO-UNDO.  
   DEFINE VARIABLE intSsnBuildLocationID AS sessionValue NO-UNDO.
   DEFINE VARIABLE intSsnPackageID       AS sessionValue NO-UNDO. 
   
   /* Clear Session */
   fClearSessionValue("PrintLabelType").
   fClearSessionValue("VerifyStockPackageUserPrompt").
   fClearSessionValue("ParentPackageID"). 
   fClearSessionValue("BuildLocationID").
   fClearSessionValue("PackageID").
   fClearSessionValue("LastScan").

   /* Frames and UI */
   DEFINE FRAME ScanPackageFrame
      SKIP(4)
      chrUserPrompt     NO-LABEL COLON 1
      chrScannedPackage NO-LABEL COLON 1
      SKIP(6)
      WITH SIDE-LABELS TITLE " Close Parent " COLOR MESSAGE ROW 8 COL 1 WIDTH 30.
   
   fTranslateWidget(FRAME ScanPackageFrame:HANDLE).
      
   chrUserPrompt = fCenterOnScreen("Scan Parent Package").
   
   /* Events */
   ON 'PF4':U ANYWHERE 
   DO:
      chrScannedPackage:SCREEN-VALUE IN FRAME ScanPackageFrame = "COMPLETE".
      RETURN NO-APPLY.
   END. /* ON 'PF4':U ANYWHERE */
   
    /* Set New Data */
   chrSsnLabelType         = fNewSessionValue("PrintLabelType").
   chrSsnUserPrompt        = fNewSessionValue("VerifyStockPackageUserPrompt").
   intSsnParentPackageID   = fNewSessionValue("ParentPackageID"). 
   intSsnBuildLocationID   = fNewSessionValue("BuildLocationID").  
   logSsnTempLabel         = fNewSessionValue("TempLabel").
   chrSsnLabelName         = fNewSessionValue("LabelName").
   intSsnPackageID         = fNewSessionValue("PackageID").
   chrSsnLastScan          = fNewSessionValue("LastScan").
   
   logSsnTempLabel:setValue(FALSE).
   chrSsnLabelName:setValue("StockPackage").
   

   Main_Block:
   DO ON ERROR UNDO Main_Block, LEAVE Main_Block:

      Scan_Block:
      REPEAT ON STOP UNDO Scan_Block, RETRY Scan_Block:
         
         /* Clear UI Variable */
         chrScannedPackage = "".
         
         /* Get scanned value from GUN */
         DISPLAY chrUserPrompt     WITH FRAME ScanPackageFrame.
         UPDATE  chrScannedPackage WITH FRAME ScanPackageFrame.
         
         chrScannedPackage = TRIM(chrScannedPackage).
         
         /* Cannot be blank */
         IF chrScannedPackage = "" OR
            chrScannedPackage = ? THEN 
         DO:
            NEXT Scan_Block.
         END. /*  IF chrScannedPackage = "" OR chrScannedPackage = ? */
         
         /* Save last scanned value into the Session Last Scan variable for the next programs */
         chrSsnLastScan:setValue(chrScannedPackage).
         
         /* if user scanned COMPLETE */
         IF chrScannedPackage = "COMPLETE" THEN 
         DO:            
            chrResult = "Continue".
            LEAVE Main_Block.
         END. /* IF chrScannedPackage = "COMPLETE" */
         
                  
         FIND FIRST StockPackage NO-LOCK
            WHERE StockPackage.PackageRef = chrScannedPackage NO-ERROR.
         IF NOT AVAILABLE StockPackage THEN
         DO:
            RUN DisplayMessage("Scan Error",
                               "Stock Package " + chrScannedPackage + " does not exist. Scan another Package.").
            NEXT Scan_Block.
         END. /* IF NOT AVAILABLE StockPackage */

         /* Cant be Detrashed */
         IF StockPackage.Detrashed <> "" THEN
         DO:
            RUN DisplayMessage("Invalid Package",
                               "Stock Package " + chrScannedPackage + " was Detrashed.  Scan another Package.").
            NEXT Scan_Block.
         END.
         
         /* Check if its a Child to Parent */
         IF StockPackage.ParentStockPackageID <> 0 THEN 
         DO:
            RUN DisplayMessage("Invalid Package",
                               "Stock Package " + chrScannedPackage + " is not a Parent.").
            NEXT Scan_Block.
         END.
         
         /* Check if Parent is open */
         FIND FIRST ParentBuild NO-LOCK 
            WHERE ParentBuild.StockPackageID = StockPackage.StockPackageID 
            AND   ParentBuild.Completed = "" NO-ERROR.
         IF NOT AVAILABLE ParentBuild THEN 
         DO:
            RUN DisplayMessage("Invalid Package",
                               "Stock Package " + chrScannedPackage + " has no open parent build record").
            NEXT Scan_Block.
         END.
               
         /* Check if package is from Kitting */
         intReturnedWorkOrderID = fGetWorkOrderIDForPalletBuild(StockPackage.StockPackageID).
                 
         IF intReturnedWorkOrderID = 0 OR intReturnedWorkOrderID = ? THEN 
         DO:
            RUN DisplayError("Invalid Package",
                             "WorkorderID returned is invalid, Scanned Parent " + chrScannedPackage + " not From Kitting").
            NEXT Scan_Block.
         END.

         /* Set Label to Print Final */
         chrSsnLabelType:setValue("RECEIPT").
         chrSsnUserPrompt:setValue("Scan Parent Final Label"). 
         intSsnParentPackageID:setValue(StockPackage.StockPackageID).
         intSsnPackageID:setValue(StockPackage.StockPackageID).  
         intSsnBuildLocationID:setValue(StockPackage.LocationID).
         
         chrResult = "Continue".
         
         LEAVE Main_Block.
      END. /* Scan Block */
   END. /* Main_Block: */     

   /* Clean Up */
   DELETE OBJECT chrSsnLastScan        NO-ERROR.
   DELETE OBJECT chrSsnUserPrompt      NO-ERROR.
   DELETE OBJECT chrSsnLabelType       NO-ERROR.
   DELETE OBJECT intSsnParentPackageID NO-ERROR.
   DELETE OBJECT logSsnTempLabel       NO-ERROR.
   DELETE OBJECT chrSsnLabelName       NO-ERROR.
   DELETE OBJECT intSsnPackageID       NO-ERROR.

   /* Release */
   RELEASE StockPackage NO-ERROR.
 
   /* Map Result Debugging */
   {prcProcessDebugging.i}

END. /* CTRL-C Catch */
